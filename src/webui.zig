...
/// Wrap a module generated by `addEmbeddedDir` from this package's build.zig.
/// File contents live in the executable; lookups allocate nothing and perform no I/O.
/// `assets.files`, `assets.map`, and the optional `assets.responses` must list
/// the same paths in bytewise order, as `addEmbeddedDir` generates them.
pub fn EmbeddedFS(comptime assets: type) type {
    return struct {
        /// Look up an exact, case-sensitive relative path using '/' separators.
        /// Returns null for missing paths; empty files return a non-null empty slice.
        pub fn get(path: []const u8) ?[]const u8 {
            if (assets.files.len == 0) return null; // comptime: no index into an empty array
            const index = indexOf(path) orelse return null;
            return assets.map[index][1];
        }

        /// Complete `HTTP/1.1 200 OK` response (headers + body) for the file handler
        /// path, e.g. `/css/main.css`; one leading '/' is stripped, nothing else is
        /// normalized. The slice is static, so WebUI sends it without copying and
        /// never frees it. Matches `setFileHandler`: `win.setFileHandler(Assets.response)`.
        /// Requires `addEmbeddedDir(..., .{ .http_responses = true })`.
        pub fn response(path: []const u8) ?[]const u8 {
            if (comptime !@hasDecl(assets, "responses")) {
                @compileError("EmbeddedFS.response requires addEmbeddedDir with .http_responses = true");
            }
            if (assets.files.len == 0) return null; // comptime: no index into an empty array
            const relative = if (std.mem.startsWith(u8, path, "/")) path[1..] else path;
            const index = indexOf(relative) orelse return null;
            return assets.responses[index];
        }

        /// All embedded file paths, sorted bytewise.
        pub fn list() []const []const u8 {
            return &assets.files;
        }

        /// Binary search: O(log n) regardless of how many paths share a length.
        fn indexOf(path: []const u8) ?usize {
            return std.sort.binarySearch([]const u8, &assets.files, path, orderPath);
        }

        fn orderPath(path: []const u8, item: []const u8) std.math.Order {
            return std.mem.order(u8, path, item);
        }
    };
}
...
fn responseSlice(buffer: []u8) WebUIError![:0]u8 {
    const len = std.mem.findScalar(u8, buffer, 0) orelse return WebUIError.ScriptError;
